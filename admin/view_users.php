...
if (isset($_POST['profile_save'])) {
    ...
    $sql = "UPDATE accounts SET firstname=:firstname,lastname=:lastname,current_balance=:current_balance,acct_email=:acct_email,acct_gender=:acct_gender,billing_code=:billing_code,transfer=:transfer,acct_currency=:acct_currency,acct_phone=:acct_phone,acct_address=:acct_address,acct_dob=:acct_dob,savings_balance=:savings_balance,loan_balance=:loan_balance,state=:state WHERE id=:id";
    $stmt = $conn->prepare($sql);
    $stmt->execute([
        'firstname' => $firstname,
        'lastname' => $lastname,
